...
function DisplayStudents(arrayOfStudents){
    arrayOfStudents.forEach(element => {
        const{name,email,course,year,marks,isGraduated}=element;
        let template  =`
        --Student University Record--
        Name: ${name}
        Email: ${email}
        Course: ${course}
        Year: ${year}
...
function StudentName(arrayOfStudents){
    arrayOfStudents.forEach(({name})=>{
        console.log(name);
    })
}
